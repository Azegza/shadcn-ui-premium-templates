...
function MagneticCard({ children, className }: { children: React.ReactNode; className?: string }) {
  ...
  return (
    <motion.div
      ...
      className={cn("group relative rounded-2xl border border-zinc-200 bg-white shadow-sm transition-all dark:border-zinc-800 dark:bg-zinc-900/50", className)}
    >
...
function StatCard({ title, value, trend, trendValue, icon: Icon, delay }: any) {
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay, duration: 0.5, ease: [0.23, 1, 0.32, 1] }}>
      <MagneticCard className="p-6">
        <div className="flex items-center justify-between">
          ...
          <Badge variant="secondary" className={cn("gap-1 font-semibold", trend === "up" ? "bg-emerald-50 text-emerald-700 dark:bg-emerald-500/10 dark:text-emerald-400" : "bg-rose-50 text-rose-700 dark:bg-rose-500/10 dark:text-rose-400")}>
        ...
        <div className="mt-4">
          ...
          <h3 className="mt-1 text-2xl font-bold tracking-tight text-zinc-900 dark:text-white">{value}</h3>
        </div>
      </MagneticCard>
    </motion.div>
  );
...
const OverviewUI = ({ data }: { data: any }) => (
  <div className="space-y-8">
    <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-4">
      <StatCard title="Total Revenue" value="$128,430" trend="up" trendValue="12.5%" icon={CurrencyDollar} delay={0.1} />
      <StatCard title="Active Users" value="43,520" trend="up" trendValue="8.2%" icon={Users} delay={0.2} />
      <StatCard title="Conversion Rate" value="3.42%" trend="down" trendValue="1.1%" icon={ChartLineUp} delay={0.3} />
      <StatCard title="Session Duration" value="4m 32s" trend="up" trendValue="24%" icon={Layout} delay={0.4} />
    </div>
    ...
    <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
          ...
          <div className="h-[300px] w-full mt-4">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={data}>
                ...
                <XAxis dataKey="name" axisLine={false} tickLine={false} tick={{ fill: "oklch(0.4 0.02 240)", fontSize: 12 }} dy={10} />
                <YAxis axisLine={false} tickLine={false} tick={{ fill: "oklch(0.4 0.02 240)", fontSize: 12 }} tickFormatter={(v) => `$${v/1000}k`} />
                <Tooltip contentStyle={{ backgroundColor: "var(--background)", borderRadius: "12px" }} />
      ...
      <Card className="rounded-2xl border-zinc-200 dark:border-zinc-800 dark:bg-zinc-900/50">
        ...
        <CardContent className="flex flex-col items-center">
          ...
          <div className="w-full space-y-2 mt-4 text-sm text-zinc-600 dark:text-zinc-300">
     ...
     <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
        <Card className="rounded-2xl border-zinc-200 dark:border-zinc-800 dark:bg-zinc-900/50">
          <CardHeader>
            ...
            <CardDescription>API requests and computational load.</CardDescription>
          </CardHeader>
          <CardContent>
             <div className="h-[300px] w-full">
                <ResponsiveContainer width="100%" height="100%">
                   <BarChart data={revenueData}>
                      <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="var(--border)" opacity={0.5} />
                      <XAxis dataKey="name" axisLine={false} tickLine={false} tick={{ fill: "oklch(0.4 0.02 240)", fontSize: 12 }} />
                      <YAxis axisLine={false} tickLine={false} tick={{ fill: "oklch(0.4 0.02 240)", fontSize: 12 }} />
        ...
        <Card className="rounded-2xl border-zinc-200 dark:border-zinc-800 dark:bg-zinc-900/50">
          <CardHeader>
            ...
            <CardDescription>User stickiness over 30 days.</CardDescription>
          </CardHeader>
          <CardContent>
             <div className="h-[300px] w-full">
                <ResponsiveContainer width="100%" height="100%">
...
const RevenueUI = () => (
  <Card className="rounded-2xl border-zinc-200 dark:border-zinc-800 dark:bg-zinc-900/50">
    <CardHeader>
      ...
      <CardDescription>Monthly breakdown of gross and net profit.</CardDescription>
    </CardHeader>
    <CardContent className="p-0">
      <div className="overflow-x-auto">
        <table className="w-full text-left">
          <thead className="bg-zinc-50 dark:bg-zinc-900/50 text-xs font-bold text-zinc-500 uppercase tracking-wider">
            <tr>
  ...
  <Card className="rounded-2xl border-zinc-200 dark:border-zinc-800 dark:bg-zinc-900/50">
    <CardHeader className="flex flex-row items-center justify-between">
      <div>
        ...
        <CardDescription>Manage your user base and their subscriptions.</CardDescription>
      </div>
    ...
    <CardContent className="p-0">
       <div className="overflow-x-auto">
        <table className="w-full text-left">
          <thead className="bg-zinc-50 dark:bg-zinc-900/50 text-xs font-bold text-zinc-500 uppercase">
            <tr>
          ...
          <tbody className="divide-y divide-zinc-100 dark:divide-zinc-800">
            {customers.map((c) => (
              <tr key={c.id} className="hover:bg-zinc-50 dark:hover:bg-zinc-800/30">
                ...
                <td className="px-6 py-4">
                  <Badge className={c.status === "Active" ? "bg-emerald-500/10 text-emerald-600" : "bg-zinc-100 text-zinc-500"}>{c.status}</Badge>
                </td>
                <td className="px-6 py-4 font-bold">{c.spent}</td>
                <td className="px-6 py-4 text-right">
                   <div className="flex justify-end gap-2">
                      <Button variant="ghost" size="icon" className="h-8 w-8"><PencilSimple /></Button>
                      <Button variant="ghost" size="icon" className="h-8 w-8 text-rose-500"><Trash /></Button>
                   </div>
                </td>
              </tr>
            ))}
  ...
  <div className="max-w-3xl space-y-6">
    <Card className="rounded-2xl border-zinc-200 dark:border-zinc-800 dark:bg-zinc-900/50">
      <CardHeader>
        ...
        <CardDescription>Update your workspace profile and identity.</CardDescription>
      </CardHeader>
      ...
      <CardContent className="space-y-6">
        <div className="flex items-center justify-between">
           <div className="space-y-0.5">
             ...
             <p className="text-xs text-zinc-600 dark:text-zinc-300">Receive weekly revenue reports via email.</p>
           </div>
        ...
        <div className="flex items-center justify-between">
           <div className="space-y-0.5">
             ...
             <p className="text-xs text-zinc-600 dark:text-zinc-300">Secure your account with 2FA verification.</p>
           </div>
           <Switch />
        </div>
      </CardContent>
...
export function MetricsDashboardBlock() {
  ...
  const renderContent = () => {
    switch (activeTab) {
      case "Overview": return <OverviewUI data={data} />;
      ...
      default: return <OverviewUI data={data} />;
    }
  };
  ...
  return (
    <div className="flex min-h-screen bg-zinc-50 dark:bg-zinc-950 font-sans">
      <aside className={cn(
        ...
        <div className="flex h-16 items-center justify-between px-6">
          {isSidebarOpen ? (
            ...
            <div className="mx-auto flex h-8 w-8 items-center justify-center rounded-lg bg-emerald-600 text-white shadow-lg shadow-emerald-500/20">
              <TrendUp weight="bold" />
            </div>
          )}
        </div>
          ...
          {[
            ...
            <Button
              ...
              className={cn(
                ...
                activeTab === item.label ? "bg-emerald-50 text-emerald-700 dark:bg-emerald-500/10 dark:text-emerald-400 font-semibold" : "text-zinc-500 hover:bg-zinc-100 dark:text-zinc-400 dark:hover:bg-zinc-800/50",
        ...
        <div className="p-4 border-t border-zinc-200 dark:border-zinc-800">
           <Button
              variant="ghost"
              className={cn("w-full justify-start gap-3 rounded-xl px-3 text-zinc-600 dark:text-zinc-300", !isSidebarOpen && "justify-center")}
      ...
      <main className={cn("flex-1 transition-all duration-300", isSidebarOpen ? "ps-64" : "ps-20")}>
        <header className="sticky top-0 z-40 flex h-16 items-center justify-between border-b border-zinc-200 bg-white/80 px-8 backdrop-blur-md dark:border-zinc-800 dark:bg-zinc-950/80">
          <div className="flex items-center gap-4 flex-1">
            <div className="relative max-w-xs w-full">
              <MagnifyingGlass className="absolute left-3 top-1/2 -translate-y-1/2 text-zinc-400" size={16} />
              <Input placeholder="Search analytics..." className="ps-10 h-9 rounded-xl border-zinc-200 bg-zinc-50 dark:border-zinc-800 dark:bg-zinc-900" />
            </div>
          </div>
          <div className="flex items-center gap-4">
            ...
            <Button variant="ghost" size="icon" className="relative rounded-full">
              ...
              <span className="absolute right-2.5 top-2.5 h-2 w-2 rounded-full bg-emerald-500 border-2 border-white dark:border-zinc-950" />
            </Button>
        ...
        <div className="p-8 space-y-8 max-w-7xl mx-auto">
          <div className="flex items-end justify-between">
            <div className="space-y-1">
              ...
              <p className="text-zinc-500 dark:text-zinc-400">Welcome back, here&apos;s what&apos;s happening with your projects today.</p>
            </div>
            {activeTab === "Overview" && (
              <div className="flex gap-2">
                ...
                <Button className="bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl shadow-lg shadow-emerald-500/20 active:scale-95 transition-all">Add Widget</Button>
              </div>
            )}
          </div>
