...
function PriceDisplay({ price, period, symbol, variant = "premium" }: { price: number; period: string; symbol: string; variant?: "standard" | "premium" }) {
...
function PricingCard({ 
  tier, isAnnual, period, currencySymbol, onSelectPlan, variant = "premium" 
}: { 
  tier: PricingTier; isAnnual: boolean; period: "monthly" | "annual"; currencySymbol: string; onSelectPlan?: (id: string, period: "monthly" | "annual") => void; variant?: "standard" | "premium" 
}) {
  const mouseX = useMotionValue(0);
  const mouseY = useMotionValue(0);
  
  // 3D Magnetic Tilt Effect (2026 standard) - Disabled in standard variant
  ...
  return (
    <motion.div
      variants={variant === "premium" ? cardVariants : {}}
      style={variant === "premium" ? { rotateX, rotateY, transformPerspective: 1200 } : {}}
    ...
    >
      ...
      <div aria-hidden="true" className={cn(
        "absolute inset-0 -z-10 rounded-2xl bg-violet-500/0 blur-xl transition-colors duration-500",
        variant === "premium" && "group-hover:bg-violet-500/15"
      )} />
      {/* Spinning gradient border — only on highlighted premium */}
      {tier.highlighted && variant === "premium" && <SpinningBorder />}
      ...
      >
        {/* Interactive Mouse Spotlight - Premium only */}
        {variant === "premium" && (
          <motion.div
        ...
        {/* Subtle glass noise texture - Premium only */}
        {variant === "premium" && (
        ...
        <div className="relative z-10">
          ...
          <PriceDisplay price={isAnnual ? tier.annualPrice : tier.monthlyPrice} period={period} symbol={currencySymbol} variant={variant} />
      ...
      <div className="relative mx-auto max-w-6xl">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, ease: SPRING }} className="text-center">
          ...
          <h2 className="text-4xl font-bold tracking-tight text-zinc-900 dark:text-white sm:text-5xl lg:text-6xl overflow-hidden pb-2">
            <motion.span
              initial={variant === "premium" ? { y: "100%", opacity: 0, filter: "blur(10px)" } : {}}
              animate={variant === "premium" ? { y: 0, opacity: 1, filter: "blur(0px)" } : {}}
            ...
            <motion.span
              ...
              className={cn("inline-block", variant === "premium" && "animate-gradient-flow bg-clip-text text-transparent")}
              style={variant === "premium" ? { backgroundImage: "linear-gradient(90deg, #7c3aed, #3b82f6, #8b5cf6, #06b6d4, #7c3aed)" } : {}}
            >
              nothing more
            </motion.span>
          </h2>
        ...
        <motion.div variants={containerVariants} initial="hidden" animate="visible"
          ...
          {resolved.map((tier) => (
            <PricingCard key={tier.id} tier={tier} isAnnual={isAnnual} period={period} currencySymbol={currencySymbol} onSelectPlan={onSelectPlan} variant={variant} />
          ))}
        </motion.div>
        {/* ── Trust footer ── */}
